// Filter Example CopyRight 2007-2008 John Robinson
// Demonstrates capturing frames from the pxCamera class and applying a filter

// Keys
// <TAB> - Switch camera sources
// <SPACE> - Switch applied filter
...
void grayscaleFilter(pxBuffer& b)
{
    for (int i = 0; i < b.height(); i++)
    {
        pxPixel *d = b.scanline(i);
        pxPixel *de = d + b.width();
        while (d < de)
        {
            d->r = d->g = d->b  = (d->r + d->g + d->b)/3;
            d++;
        }
    }
}

void threshholdFilter(pxBuffer& b)
{
    grayscaleFilter(b);
...
private:
    ...
    void onCameraCapture(pxBuffer& frame)
    {
        ...
        // Apply a filter to the pxBuffer
        switch (mCurrentFilter)
        {
            case 0:  // no filter
                break;
            case 1:
                grayscaleFilter(frame);
                break;
            case 2:
                threshholdFilter(frame);
                break;
            default: // no filter
                break;
        }
    ...
    void updateTitle()
    {
        ...
        if (mCamera)
        {
            sprintf(buffer, "Filter - %s - ", mCamera.name());

            switch(mCurrentFilter)
            {
                case 1:
                    strcat(buffer, "GRAYSCALE FILTER");
                    break;
                case 2:
                    strcat(buffer, "THRESHOLD FILTER");
                    break;
                default:
            ...
            strcpy(buffer, "Filter - Please attach a camera and then press <TAB>");
    ...
    void changeFilter()
    {
        mCurrentFilter = (mCurrentFilter + 1) % MAXFILTER;
